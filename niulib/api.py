...
logger = logging.getLogger("niulib_api")


class NiuApi:
    def __init__(self, username, password, scooter_id, *, md5=None) -> None:
    ...
    def expired(self):
        return datetime.datetime.now().timestamp() >= self.expires_time + 60

    def is_acc_on(self):
        return self.dataMoto["data"]["isAccOn"] != 0

    def toggleAcc(self):
        if self.updateMoto().is_acc_on():
            logger.info("will switch off")
            self.action("acc_off")
        else:
            logger.info("will switch on")
            self.action("acc_on")

    def initApi(self):
        self.token, self.expires_time = self.get_token()
    ...
    def get_token(self):
        ...
        return data["data"]["token"]["access_token"], data["data"]["token"]["token_expires_in"]
    ...
    def action(self, cmd: str):
        ...
        try:
            ...
            logger.exception()
        data = json.loads(r.content.decode())
        logger.info(f"Perform action {cmd}")
        return data
    ...
    def getDataTrack(self, id_field):
        if id_field == "startTime" or id_field == "endTime":
            return datetime.fromtimestamp((self.dataTrackInfo["data"][0][id_field]) / 1000).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        if id_field == "ridingtime":
            return strftime("%H:%M:%S", gmtime(self.dataTrackInfo["data"][0][id_field]))
        if id_field == "track_thumb":
            thumburl = self.dataTrackInfo["data"][0][id_field].replace("app-api.niucache.com", "app-api-fk.niu.com")
